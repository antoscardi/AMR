...
for k = 1:Nstep
% Sensitivity
f_p(:,:,k) = ff_p(q_history(:,k),u_history(:,k),params(:,k));
f_q(:,:,k) = ff_q(q_history(:,k),u_history(:,k),params(:,k));
f_u(:,:,k) = ff_u(q_history(:,k),params(:,k));
h_q(:,:,k) = fh_q(q_history(:,k),xhi_history(:,k),desired_traj(:,k),dp(:,k),ddp(:,k));
h_xhi(:,:,k) = fh_xhi(q_history(:,k),xhi_history(:,k),desired_traj(:,k),dp(:,k),ddp(:,k));
...
% Gamma
h_ax1(:,:,k) = h_a_1x(q_history(:,k),time(k),xhi_history(:,k));
h_ay1(:,:,k) = h_a_1y(q_history(:,k),time(k),xhi_history(:,k));
h_ax2(:,:,k) = h_a_2x(q_history(:,k),time(k),xhi_history(:,k));
h_ay2(:,:,k) = h_a_2y(q_history(:,k),time(k),xhi_history(:,k));
h_ax3(:,:,k) = h_a_3x(q_history(:,k),time(k),xhi_history(:,k));
h_ay3(:,:,k) = h_a_3y(q_history(:,k),time(k),xhi_history(:,k));
h_ax4(:,:,k) = h_a_4x(q_history(:,k),time(k),xhi_history(:,k));
h_ay4(:,:,k) = h_a_4y(q_history(:,k),time(k),xhi_history(:,k));
g_ax1(:,:,k) = g_a_1x(q_history(:,k),time(k));
g_ay1(:,:,k) = g_a_1y(q_history(:,k),time(k));
g_ax2(:,:,k) = g_a_2x(q_history(:,k),time(k));
g_ay2(:,:,k) = g_a_2y(q_history(:,k),time(k));
g_ax3(:,:,k) = g_a_3x(q_history(:,k),time(k));
g_ay3(:,:,k) = g_a_3y(q_history(:,k),time(k));
...
for k=1:Nstep
    dhqa1x(:,:,k) = dhq_a1x(time(k), q_history(3,k), xhi_history(1,k));
    dhqa2x(:,:,k) = dhq_a2x(time(k), q_history(3,k), xhi_history(1,k));
    dhqa3x(:,:,k) = dhq_a3x(time(k), q_history(3,k), xhi_history(1,k));
    dhqa4x(:,:,k) = dhq_a4x(q_history(3,k), xhi_history(1,k));
    dhqa1y(:,:,k) = dhq_a1y(time(k), q_history(3,k), xhi_history(1,k));
    dhqa2y(:,:,k) = dhq_a2y(time(k), q_history(3,k), xhi_history(1,k));
    dhqa3y(:,:,k) = dhq_a3y(time(k), q_history(3,k), xhi_history(1,k));
    dhqa4y(:,:,k) = dhq_a4y(q_history(3,k), xhi_history(1,k));

    dhxhia1x(:,:,k) = dhxhi_a1x(time(k), q_history(3,k), xhi_history(1,k));
    dhxhia2x(:,:,k) = dhxhi_a2x(time(k), q_history(3,k), xhi_history(1,k));
    dhxhia3x(:,:,k) = dhxhi_a3x(time(k), q_history(3,k), xhi_history(1,k));
    dhxhia4x(:,:,k) = dhxhi_a4x(q_history(3,k), xhi_history(1,k));
    dhxhia1y(:,:,k) = dhxhi_a1y(time(k), q_history(3,k), xhi_history(1,k));
    dhxhia2y(:,:,k) = dhxhi_a2y(time(k), q_history(3,k), xhi_history(1,k));
    dhxhia3y(:,:,k) = dhxhi_a3y(time(k), q_history(3,k), xhi_history(1,k));
    dhxhia4y(:,:,k) = dhxhi_a4y(q_history(3,k), xhi_history(1,k));

    dgqa1x(:,:,k) = dgq_a1x(time(k), q_history(3,k));
    dgqa2x(:,:,k) = dgq_a2x(time(k), q_history(3,k));
    dgqa3x(:,:,k) = dgq_a3x(time(k), q_history(3,k));
    dgqa4x(:,:,k) = dgq_a4x(q_history(3,k));
    dgqa1y(:,:,k) = dgq_a1y(time(k), q_history(3,k));
    dgqa2y(:,:,k) = dgq_a2y(time(k), q_history(3,k));
    dgqa3y(:,:,k) = dgq_a3y(time(k), q_history(3,k));
