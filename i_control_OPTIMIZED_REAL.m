...
r_d_opt = optimr_d_; dr_d_opt = optimdr_d; ddr_d_opt = optimddr_d;
...
%% Create and display video animation and plots for the NOMINAL trajectory.
% The video function just needs the distance between the wheels in order to plot the robot.
b_n = perturbed_params(2); 
video(q_history,r_d,b_n,timeVec,linewidth,delta, 'Non Optimized Trajectory')

% Plot state variables (vector q).
plot_function(q_history,'State variation in time Nominal','x [m] ; y [m] ; theta [rad/s]', timeVec, linewidth, colors, counter) 
% Plot input (vector u).
plot_function(u_history,'Input variation in time Nominal','wr [rad/s] ; wl [rad/s]', timeVec, linewidth, colors, counter)
% Plot errors.
plot_function([e;e_tot'],'Error variation over time Nominal','e_x [m] ; e_y [m] ; e_tot [m]',timeVec, linewidth, colors, counter)


%% Create and display video animation and plots for the OPTIMAL trajectory.
% The video function just needs the distance between the wheels in order to plot the robot.
b_n = perturbed_params(2); 
video(q_history_opt,r_d_opt,b_n,timeVec,linewidth,delta, 'Optimal Trajectory')

% Plot state variables (vector q).
plot_function(q_history_opt,'State variation in time Optimal ','x [m] ; y [m] ; theta [rad/s]', timeVec, linewidth, colors, counter) 
% Plot input (vector u).
plot_function(u_history_opt,'Input variation in time Optimal','wr [rad/s] ; wl [rad/s]', timeVec, linewidth, colors, counter)
% Plot errors.
plot_function([e_opt; e_tot_opt'],'Error variation over time Optimal','e_x [m] ; e_y [m] ; e_tot [m]',timeVec, linewidth, colors, counter)
