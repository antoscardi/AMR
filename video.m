function video(q_history,p,R,time)
% Setting up the Plot
...
trajectory_d = plot(p(1,1:1), p(2,1:1));
orient = line([p_r(1,1),p_r(1,1)+R*cos(theta(1))],[p_r(2,1), p_r(2,1)+R*sin(theta(1))],'Color',colors(3,:),"LineStyle","-."); orient.MarkerSize = 6;
...
for k = 2:length(time)
    ...
    trajectory_d.XData = p(1,1:k);
    trajectory_d.YData = p(2,1:k);
